...
- PRD는 H1을 제외한 모든 제목에 `1.`, `1.1.` 형식의 계층 번호를 붙인다.
- GitHub 기능 구현 이슈는 `.github/ISSUE_TEMPLATE/feature.yml` 템플릿을 따른다.
  - 기능 구현 이슈 제목에는 `[feature]` 같은 타입 prefix를 붙이지 않는다.
  - 기능 구현 이슈 라벨은 `feature`와 `docs/features/` 하위 기능명과 같은 라벨 1개를 함께 붙인다. 예: `feature`, `tag`
  - 기능 구현 이슈의 `의존성`에는 선행 이슈나 함께 머지되어야 하는 다른 이슈만 작성한다.
